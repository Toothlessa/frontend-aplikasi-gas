<template>
  <nav>
    <v-navigation-drawer v-model="drawer" :rail="rail" permanent @click="rail = false" app dark color="teal">
      <v-list-item lines="two" prepend-icon="mdi-face-profile" :title="result.username" :subtitle="result.email" nav>
        ...
        <v-list-item-group v-model="selectedItem" color="white">
          ...
          <v-list-group value="Master">
            <template v-slot:activator="{ props }">
              <v-list-item v-bind="props" title="Master" prepend-icon="mdi-chart-pie" :rail="rail"
                @click="rail = false"></v-list-item>
            </template>

            <v-list-item v-for="([icon, title, to], i) in masters" :key="i" :prepend-icon="icon" :title="title"
              :value="title" :to="to"></v-list-item>
          </v-list-group>
          <v-list-item v-for="(item, i) in items" :key="i" :value="item" :to="item.to"
    ...
    <v-dialog v-model="dialogLogout" max-width="400px">
      <v-card class="elevation-12" variant="elevated">
        <v-card-title class="bg-teal text-white text-center">Apa anda yakin ingin Logout?</v-card-title>
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="teal" variant="text" @click="closeLogout">
            Tidak
          </v-btn>
          <v-btn color="teal" variant="text" @click="onLogout">
            Ya
          </v-btn>
          <v-spacer></v-spacer>
        </v-card-actions>
